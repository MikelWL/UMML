...
if config.DATASET == "MIRFlickr":
    ...
    indexTest = np.arange(18014, 20014, 1)
    indexDatabase = np.arange(0, 18014, 1)
    indexTrain = np.arange(0, 9999, 1)
...
if config.DATASET == "NUSWIDE":
    ...
    txt_file = scio.loadmat(config.TXT_DIR)
    txt_set = np.array(txt_file['YAll'])
    ...
    indexTest = np.arange(193733, 195833, 1)
    indexDatabase = np.arange(0, 193733, 1)
    indexTrain = np.arange(0, 9999, 1)

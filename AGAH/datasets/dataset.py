...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, opt, images, tags, labels, test=None):
        self.test = test
        if test is None:
            train_images = images[:opt.training_size]
            train_tags = tags[:opt.training_size]
            train_labels = labels[:opt.training_size]
            ...
            self.query_labels = labels[opt.db_size:]
            self.db_labels = labels[:opt.db_size]
            if test == 'image.query':
                #self.images = images[0: opt.query_size]
                self.images = images[opt.db_size:]
            elif test == 'image.db':
                self.images = images[:opt.db_size]
            elif test == 'text.query':
                #self.tags = tags[0: opt.query_size]
                self.tags = tags[opt.db_size:]
            elif test == 'text.db':
                self.tags = tags[:opt.db_size]
